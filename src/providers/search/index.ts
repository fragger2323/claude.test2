...
export function businessNameFromTitle(title: string): string {
  ...
  if (parts.length === 0) return title.trim().slice(0, 80);
  // prefer the shortest segment that is not a generic phrase
  const generic = /^(home|strona główna|startseite|accueil|inicio|главная|головна|welcome|witamy|kontakt|contact|o nas|about us)$/i;
  const candidates = parts.filter((p) => !generic.test(p));
  return (candidates.sort((a, b) => a.length - b.length)[0] ?? parts[0]!).slice(0, 80);
}
...
export class WebDiscoveryAdapter implements LeadSourceAdapter {
  ...
  async searchBusinesses(q: BusinessQuery, ctx: ProviderContext): Promise<SearchResultPage> {
    ...
    for (const r of results) {
      ...
      if (classifyNonOfficialDomain(url)) continue;
      seen.add(domain);
      const origin = new URL(url).origin;
      items.push({
        provider: this.id,
        providerRecordId: domain,
        name: businessNameFromTitle(r.title),
        categories: [q.term],
        city: q.city,
  ...
  async findWebsite(b: BusinessLookup, ctx: ProviderContext): Promise<WebsiteCandidate[]> {
    ...
    return results
      .filter((r) => !classifyNonOfficialDomain(r.url) && normalizeUrl(r.url))
      .slice(0, 5)
      .map((r) => ({
